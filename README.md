...
```mermaid
flowchart LR
    A["Очистка<br/>выбросов"] --> B["Спрос без<br/>дефицита"] --> C["Сезон<br/>и тренд"] --> D["Заказ<br/>+ страх. запас"] --> E["Дата<br/>заказа"]
...
```mermaid
flowchart LR
    N["Крупная<br/>накладная"] --> Q{"Повторяется<br/>в 4+ месяцах?"}
    Q -- да --> K["Оптовик:<br/>оставить"]
    Q -- нет --> X["Разовый:<br/>исключить"]
