...
## 📨 Заказ поставщику за три шага

На главном экране — карточка каждого поставщика: сколько позиций заказать сегодня, штук и сумма по известным ценам. Кнопка **«Проверить и согласовать заказ»** открывает готовый заказ:

1. **Выбрать** — менеджер сам решает, что покупать: галочки у каждой позиции, поиск по названию, коду 1С или артикулу, кнопка «+ Добавить» для любого товара поставщика, правка количества. Можно взять только срочное, добавить позиции этой недели или весь заказ. Итоги, Excel и сообщение считаются только по отмеченным позициям.
2. **Согласовать** — отметка на этом устройстве.
3. **Отправить** — кнопки **WhatsApp**, **Telegram**, **Почта**: открывается мессенджер с готовым текстом заказа (итоги и первые 15 позиций), Excel скачивается для вложения. На телефоне кнопка **«Поделиться файлом»** отправляет Excel сразу.

Сервис **ничего не отправляет сам**: получателя выбирает менеджер, кнопки отправки появляются только после согласования. Коммерческие данные лучше отправлять с рабочего номера.
