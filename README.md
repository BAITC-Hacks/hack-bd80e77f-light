...
### Что сервис нашёл в реальных данных партнёра

2 944 активных артикула IEK и Systeme Electric, 33 месяца истории, 248 тыс. строк накладных:

| | Находка | Что это значит для бизнеса |
|:---:|---|---|
| 📦 | **1 226 позиций к заказу** | готовый заказ по двум поставщикам вместо ручного расчёта |
| ⏰ | **1 025 позиций нужно заказать сегодня**, из них 473 закончатся раньше, чем придёт поставка | у каждого товара есть дата «заказать до» |
| 🧊 | **1 290 позиций с излишком**, **30,2 млн ₸** заморожено в запасах SE | запаса больше чем на 6 месяцев: заказывать не нужно |
| 🚫 | **1 054 разовых крупных заказа** (127 тыс. шт) исключены | без фильтра они раздули бы регулярную закупку |
| 📉 | **+19,6 тыс. шт упущенного спроса** восстановлено | когда товара не было, продажи занижены, и прогноз это учитывает |

![Главный экран: заказ по поставщикам, срочность, дата «заказать до», обоснование](docs/dashboard.png)
...
## ⏰ Главное: не только «сколько», но и «когда» заказать

Поставка идёт **~45 дней**. Если опоздать с заказом на неделю, неделя без товара гарантирована, и количество этого уже не исправит. Поэтому по каждому товару сервис строит **календарь остатка на 6 месяцев**: день за днём вычитает прогноз спроса (с сезонностью и трендом) и прибавляет приходы товара в пути по датам из файлов поставщика.

| | Как считается | Что видит менеджер |
|---|---|---|
| 📅 **Закончится** | первый день, когда остаток без нового заказа станет 0 | «ВА47-29 10А закончится 19 окт» |
| ⏰ **Заказать до** | день, когда остаток опустится ниже страхового запаса, **минус срок поставки** | «до 30 сен», «Сегодня» или «опоздание 12 дн» |
| 🔁 **Следующий заказ** | то же, но с учётом сегодняшнего заказа | «Следующий заказ — до 20 дек» |

```text
Остаток(день) = Остаток(вчера) + приход товара в пути (по дате) − спрос(день)
Заказать до   = первый день, когда Остаток < страхового запаса  −  срок поставки
```

Вкладка **«Календарь заказов»** раскладывает позиции по срокам: *сегодня · эта неделя · следующая неделя · этот месяц · 1–2 месяца · позже*. Для каждой группы показаны сумма и потери, если не заказать вовремя. Если дефицита уже не избежать, сервис говорит это прямо и предлагает варианты: срочная доставка, перемещение, аналог.

![Календарь заказов по неделям](docs/calendar.png)

![Карточка товара: календарь остатка «без заказа / с заказом», история спроса, разовые заказы, дефицит](docs/drawer.png)
...
Если крупные продажи повторяются (в 4 и более месяцах или в > 20% месяцев продаж), алгоритм считает их регулярным оптовым спросом постоянного клиента и не исключает. Оставшиеся месячные всплески дополнительно сглаживаются фильтром Хампеля.
...
| 🟢 **Планово** | Потребность положительная, срочного риска нет | Добавить в плановый заказ до указанной даты |

Для каждого статуса рядом стоит дата **«заказать до»** из календаря остатка.
...
| **Сверх ТЗ:** когда заказывать | Календарь остатка по дням, дата «заказать до», приход товара в пути по датам | ✅ |
| Запрет автоотправки поставщику | В коде нет отправки — только утверждение и выгрузка | ✅ |
...
Самый быстрый способ — открыть файл `web/index.html` в браузере двойным щелчком (сервер не нужен). Или через локальный сервер:
...
| Графики и аналитика | SVG, без внешних библиотек |
...
- [x] Календарь остатка и дата «заказать до»
- [ ] Уведомления в Telegram: «кабель закончится 14 окт — заказать до 30 сен»
- [ ] Надёжность поставщика: страховой запас с учётом опозданий поставки
- [ ] Заявки под проекты от отдела продаж и аналоги IEK ↔ Systeme Electric
