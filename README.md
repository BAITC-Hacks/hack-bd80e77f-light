...
**Сервис умеет отправлять заказ поставщику в WhatsApp, Telegram и на почту, а также выгружать его в Excel.** Менеджер сам выбирает, какие товары покупать.

![Заказ поставщику: выбор товаров и кнопки «Согласовать и отправить в WhatsApp / Telegram»](docs/send-order.png)

**Где кнопки:**
- в шапке сайта — **«Отправить заказ»** (значки WhatsApp и Telegram);
- на карточке каждого поставщика — **«☑ Выбрать товары»**, **WhatsApp**, **Telegram**;
- в окне заказа — **«Согласовать и отправить в WhatsApp»**, **«Согласовать и отправить в Telegram»**, **«Почта»**, **«Excel»**, на телефоне ещё **«Поделиться файлом»**.

**Ручной выбор товаров** — кнопка **«☑ Выбрать товары»**:
- галочка у каждой позиции: снятые не попадают в заказ; галочка в шапке — выбрать или снять все;
- поиск по названию, коду 1С или артикулу; товары поставщика, которых нет в заказе, можно добавить кнопкой **«+ Добавить»** — даже если сервис их не рекомендовал;
- количество правится прямо в списке; можно взять только срочное, добавить позиции этой недели или весь заказ;
- итоги, Excel и текст сообщения считаются только по отмеченным позициям; выбор сохраняется в браузере.

**Как происходит отправка:** нажатие «Согласовать и отправить» — это подтверждение менеджера. Заказ отмечается согласованным, и открывается WhatsApp или Telegram с готовым текстом: поставщик, количество позиций и штук, сумма по известным ценам и **весь список позиций** (если их до 50; иначе первые 30 и ссылка на Excel). Получателя выбирает менеджер. Ничего не скачивается без спроса: файл Excel для поставщика — отдельной кнопкой «Excel», его можно приложить к сообщению. На телефоне «Поделиться файлом» отправляет Excel сразу.

**Сервис ничего не отправляет сам** — как требует ТЗ, заказ уходит только после действия ответственного сотрудника. Коммерческие данные лучше отправлять с рабочего номера.
